...
double ALG::vector::dot(const ALG::vector &src) {
    if (size() != src.size()) printError("ALG::vector::dot", "The sizes of the two vectors do not match.");
    double sum{};
...
ALG::vector ALG::vector::cross(const ALG::vector &src) {
    return ALG::vector(std::vector<double>{at(1) * src.at(2) - at(2) * src.at(1), at(2) * src.at(0) - at(0) * src.at(2),
