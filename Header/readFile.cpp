...
void ALG::readFile::readGeometryInfomation(const std::string &filename, std::vector<section> *sections, std::vector<region> *regions) {
  std::ifstream f(filename);
  std::string word{}, name{};

  if (!f.is_open()) {
    printError("ALG::readFile::readGeometryInformation", "No Geometry information file: \"%s\"\nPlease check file name", filename.c_str());
  }
  std::cout << "Geometry information file: \"" << filename << "\" open" << '\n';
...
void ALG::readFile::readSection(std::ifstream &f, std::vector<section> *sections) {
  ...
  while (word != "ENDSECTION") {
    if (word == "LINE") {
      ...
      if (line.getCondition() != 'I') {
        line.setBoundaryValue(std::stod(bv));
      }
      line.calcProperties();
      s.emplace_back(line);
    }
...
void ALG::readFile::readRegion(std::ifstream &f, std::vector<region> *regions, std::vector<section> *sections) {
  ...
  f >> name;
  std::cout << "REGION \"" << name << "\" Reading..." << '\n';
  f >> eps >> xmin >> ymin >> xmax >> ymax >> hx >> hy;

  auto r{region(name, std::stod(eps), std::stod(xmin), std::stod(xmax), std::stod(ymin), std::stod(ymax), std::stod(hx), std::stod(hy))};
