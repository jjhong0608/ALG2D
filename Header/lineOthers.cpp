...
ALG::axialLine2D::axialLine2D(const ALG::boundaryPoint &sp, const ALG::boundaryPoint &ep) : line2D(sp.at(0), sp.at(1), ep.at(0), ep.at(1)) {}
...
void ALG::axialLine2D::isinBoundary(std::deque<boundaryPoint> *boundaryPts) {
  auto checkVertice = [](boundaryPoint *point) -> bool {
    auto check = [](boundaryPoint &point1, std::vector<double> &vector1) -> bool {
      return iszero(std::sqrt((point1[0] - vector1[0]) * (point1[0] - vector1[0]) + (point1[1] - vector1[1]) * (point1[1] - vector1[1])));
    };
  ...
  for (auto &i : boundaries[0]->getLine()->getBoundaryPoints()) {
    if ((*boundaries[0] - *i).norm() < NEARZERO) {
      boundaries[0] = i;
      tf = false;
      break;
    }
  ...
  if (checkVertice(boundaries[0])) {
    for (auto &item : *boundaryPts) {
      if ((*boundaries[0] - item).norm() < NEARZERO) {
        boundaries[0] = &item;
        tf = false;
        break;
      }
  ...
  for (auto &i : boundaries[1]->getLine()->getBoundaryPoints()) {
    if ((*boundaries[1] - *i).norm() < NEARZERO) {
      boundaries[1] = i;
      return;
    }
  }
  if (checkVertice(boundaries[1])) {
    for (auto &item : *boundaryPts) {
      if ((*boundaries[1] - item).norm() < NEARZERO) {
        boundaries[1] = &item;
        return;
      }
    }
...
void ALG::gridLine2D::sortCrossSectionPts(double sign) {
  ...
  if (cross_section_pts.size() % 2 != 0) {
    ...
    cross_section_pts.erase(std::unique(cross_section_pts.begin(), cross_section_pts.end(), [](const point &a, const point &b) { return iszero((a - b).norm()); }), cross_section_pts.end());
    if (cross_section_pts.size() % 2 != 0) {
      printf("Found odd number of points intersecting section:\n");
      printf("\tGrid Line Information:\n");
      ...
      printf("Please Type [F]irst or [L]ast to remove intersection point\n");
      while (true) {
        getline(std::cin, s);
        trim(s);
        ...
        if (s == "f" || s == "first") {
          ...
          cross_section_pts.erase(cross_section_pts.end());
          break;
        } else {
          printf("Your Input is: %s, please type again\n", s.c_str());
        }
      }
    }
  } else {
...
void ALG::gridLine2D::makeAxialLine() {
  ...
  for (auto &i : cross_pts) {
    if (i.getIo() == 'I') {
      ...
      if (countIO == 0) {
        ep = &i;
        if (isclose((*sp)[0], (*ep)[0]))
          (*sp)[0] = (*ep)[0] = pts[0]->at(0);
        if (isclose((*sp)[1], (*ep)[1]))
          (*sp)[1] = (*ep)[1] = pts[0]->at(1);
  ...
  if (axial_lines.size() > 1) {
    for (int i = 0; i < axial_lines.size() - 1; ++i) {
      if ((axial_lines[i].getAnEnd() - axial_lines[i + 1].getStart()).norm() < NEARZERO) {
  ...
  for (auto &i : axial_lines) {
    if (!i.getPts().empty()) {
      if ((*(i.getBoundaries()[0]) - *(i.getPts().front())).norm() < NEARZERO) {
        i.getPts().erase(i.getPts().begin());
      }
      if ((*(i.getBoundaries()[1]) - *(i.getPts().back())).norm() < NEARZERO) {
        i.getPts().pop_back();
      }
    }
  }
...
void ALG::gridLine2D::makeAxialLine(double h) {
  ...
  for (const auto &i : pts) {
    if (it->getStart() < *i && *i < it->getAnEnd()) {
      ...
      if (it == axial_lines.end()) {
        break;
      } else if (it->getStart() < *i && *i < it->getAnEnd()) {
        it->getPts().emplace_back(i);
      }
    }
  }
  for (auto &i : axial_lines) {
    if (!i.getPts().empty()) {
      if ((*(i.getBoundaries()[0]) - *(i.getPts().front())).norm() < h / 20.) {
      ...
      if ((*(i.getBoundaries()[1]) - *(i.getPts().back())).norm() < h / 20.) {
...
ALG::boundaryLine2D::boundaryLine2D(double s0, double s1, double e0, double e1, char condition, double boundaryValue) : line2D(s0, s1, e0, e1), condition(condition), boundary_value(boundaryValue) {}

ALG::boundaryLine2D::boundaryLine2D(const ALG::vector &start, const ALG::vector &anEnd, char condition, double boundaryValue) : line2D(start, anEnd), condition(condition), boundary_value(boundaryValue) {}
